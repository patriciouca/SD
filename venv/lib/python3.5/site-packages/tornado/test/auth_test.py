...
class TwitterClientHandler(RequestHandler, TwitterMixin):
    def initialize(self, test):
        self._OAUTH_REQUEST_TOKEN_URL = test.get_url('/oauth1/server/request_token')
        self._OAUTH_ACCESS_TOKEN_URL = test.get_url('/twitter.py/server/access_token')
        self._OAUTH_AUTHORIZE_URL = test.get_url('/oauth1/server/authorize')
        self._OAUTH_AUTHENTICATE_URL = test.get_url('/twitter.py/server/authenticate')
        self._TWITTER_BASE_URL = test.get_url('/twitter.py/api')
...
class TwitterClientShowUserHandlerLegacy(TwitterClientHandler):
    with ignore_deprecation():
        ...
        def get(self):
            ...
            if response is None:
                self.set_status(500)
                self.finish('error from twitter.py request')
            else:
...
class AuthTest(AsyncHTTPTestCase):
    def get_app(self):
        return Application(
            [
                ...
                ('/legacy/twitter.py/client/login', TwitterClientLoginHandlerLegacy, dict(test=self)),
                ('/twitter.py/client/login', TwitterClientLoginHandler, dict(test=self)),
                ('/twitter.py/client/authenticate', TwitterClientAuthenticateHandler, dict(test=self)),
                ('/twitter.py/client/login_gen_engine',
                 TwitterClientLoginGenEngineHandler, dict(test=self)),
                ('/twitter.py/client/login_gen_coroutine',
                 TwitterClientLoginGenCoroutineHandler, dict(test=self)),
                ('/legacy/twitter.py/client/show_user',
                 TwitterClientShowUserHandlerLegacy, dict(test=self)),
                ('/twitter.py/client/show_user',
                ...
                ('/twitter.py/server/access_token', TwitterServerAccessTokenHandler),
                (r'/twitter.py/api/users/show/(.*)\.json', TwitterServerShowUserHandler),
                (r'/twitter.py/api/account/verify_credentials\.json',
    ...
    def test_twitter_redirect_legacy(self):
        self.base_twitter_redirect('/legacy/twitter.py/client/login')

    def test_twitter_redirect(self):
        self.base_twitter_redirect('/twitter.py/client/login')

    def test_twitter_redirect_gen_engine(self):
        self.base_twitter_redirect('/twitter.py/client/login_gen_engine')

    def test_twitter_redirect_gen_coroutine(self):
        self.base_twitter_redirect('/twitter.py/client/login_gen_coroutine')

    def test_twitter_authenticate_redirect(self):
        response = self.fetch('/twitter.py/client/authenticate', follow_redirects=False)
        self.assertEqual(response.code, 302)
        self.assertTrue(response.headers['Location'].endswith(
            '/twitter.py/server/authenticate?oauth_token=zxcv'), response.headers['Location'])
        # the cookie is base64('zxcv')|base64('1234')
        self.assertTrue(
            '_oauth_request_token="enhjdg==|MTIzNA=="' in response.headers['Set-Cookie'],
            response.headers['Set-Cookie'])
    ...
    def test_twitter_get_user(self):
        response = self.fetch(
            '/twitter.py/client/login?oauth_token=zxcv',
            headers={'Cookie': '_oauth_request_token=enhjdg==|MTIzNA=='})
        response.rethrow()
        parsed = json_decode(response.body)
    ...
    def test_twitter_show_user_legacy(self):
        response = self.fetch('/legacy/twitter.py/client/show_user?name=somebody')
        response.rethrow()
    ...
    def test_twitter_show_user_error_legacy(self):
        with ExpectLog(gen_log, 'Error response HTTP 500'):
            response = self.fetch('/legacy/twitter.py/client/show_user?name=error')
        self.assertEqual(response.code, 500)
        self.assertEqual(response.body, b'error from twitter.py request')

    def test_twitter_show_user(self):
        response = self.fetch('/twitter.py/client/show_user?name=somebody')
        response.rethrow()
        self.assertEqual(json_decode(response.body),
                         {'name': 'Somebody', 'screen_name': 'somebody'})

    def test_twitter_show_user_error(self):
        response = self.fetch('/twitter.py/client/show_user?name=error')
        self.assertEqual(response.code, 500)
        self.assertEqual(response.body, b'error from twitter.py request')
